...
RDBMS describes the entire relation database management system which is used to handle everything involved with storing
and managing data.  SQL is a specific language used to interact with the RDBMS.

1. Why do tables need a `primary key`?

Tables need primary keys to give each row a guaranteed unique field which can be used to reference and differentiate.
...
A join table which includes foreign keys referencing each primary key from the two related tables.
